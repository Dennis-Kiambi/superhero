...
public class Sql2oHeroDAO implements HeroDAO{
    ...
    public void addHero(Hero hero) {
     String sql = "INSERT INTO heroes (heroName, heroPower, heroWeakness, heroAge, squadId) VALUES (:heroName, :heroPower, :heroWeakness, :heroGender, :heroAge, :squadId)";
     try (Connection con = sql2o.open()){
    ...
    public void updateHero(int id, String name, String power, String weakness, int age, int squadId) {
        String sql = "UPDATE heroes SET heroName = :name, heroAge = :age, heroPower = :power, heroWeakness = :weakness, squadId = :squadId WHERE id=:id";
        try (Connection con = sql2o.open()) {
            con.createQuery(sql)
                    .addParameter("id",id)
